...
function showTable() {
	...
	if (mysqli_num_rows($result) > 0) {
	    ...
	    echo "<table border='1' align='center'><tr> <th>id</th> <th>file location</th> </tr>";
	    while($row = mysqli_fetch_assoc($result)) {
	    	//echo '<tr> <td>'.$row["id"].'</td> <td>'. $row["file"].'</td>  </tr>';
	    	echo "<tr> <td>".$row['id'].'</td><td>';

	    	echo "<li><a href='viewPDFFileUsingTable.php?id=".$row['id']."' target='_blank'>".$row['file']."</a></li>".'</td></tr>';
