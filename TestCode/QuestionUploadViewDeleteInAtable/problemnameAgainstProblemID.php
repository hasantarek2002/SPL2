<?php
	...
	if(isset($_REQUEST["delId"])){
		...
		header('location:problemnameAgainstProblemID.php');
		//showTable();
...
function showTable() {
	...
	if (mysqli_num_rows($result) > 0) {
	    ...
	    while($row = mysqli_fetch_assoc($result)) {
	    	...
	    	echo "<a href='problemnameAgainstProblemID.php?delId=".$row['id']."' >"."Delete"."</a>".'</td></tr>';
...
if(isset($_POST["submit"])) {
	...
	$sql5="SELECT id FROM fileUpload ORDER BY id DESC";
	...
	if($fileType == "txt"){
		...
		$f= 'q'.$renameFile.'.txt';
		@rename(basename($_FILES["fileToUpload"]["name"]), $f);
